...
var vamp = preload("res://Vampiro.tscn")
var drag = preload("res://Dragao.tscn")
...
func _on_geravampiro_timeout():
	var novovampiro = vamp.instance()
	add_child(novovampiro)
	novovampiro.position.x = rand_range(0, 4000)
	novovampiro.position.y = rand_range(0, 480)
	
func _on_geradragao_timeout():
	var novodragao = drag.instance()
	add_child(novodragao)
	novodragao.position.x = rand_range(0, 4000)
	novodragao.position.y = rand_range(0, 480)



func _on_proxfase_timeout():
